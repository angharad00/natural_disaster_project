...
```{r echo=FALSE, fig.height=6, fig.width=10}
# Create hover text including region and subtype information
hover_text <- paste("Region: ", full_data$region,
                    "<br>Type: ", full_data$type,
                    "<br>Subtype: ", full_data$subgroup,
                    "<br>Climate Effect: ", full_data$climate_change_effect)



p <- ggplot(full_data, aes(x = year, y = after_stat(count), color = type, text = hover_text)) +
    geom_line(stat = "count") +
  facet_wrap(~ region)
...
p <- ggplot(full_data, aes(x = year, y = after_stat(count), color = climate_change_effect, text = hover_text)) +
    geom_line(stat = "count") +
  facet_wrap(~ region)
...
p <- ggplot(full_data, aes(y = year, x = after_stat(count), color = climate_change_effect, text = hover_text)) +
    geom_bar(stat = "count") +
  facet_wrap(~ region)
...
```
```{r, echo=FALSE, fig.height=6, fig.width=10}
 ggplot(full_data, aes(y = year, x = after_stat(count), color = climate_change_effect, text = hover_text)) +
...
p <- ggplot(full_data, aes(x = year,  y = after_stat(count), fill = interaction(climate_change_effect), text = hover_text)) +
  geom_bar(position = "fill") +
  facet_wrap(~ region) +
  labs(title = "Natural Disasters by Climate Change Effect and Type",
...
p <- ggplot(full_data, aes(x = year,  y = after_stat(count), fill = interaction(climate_change_effect), text = hover_text)) +
  ...
  scale_fill_manual(values = c("grey", "blue", "light blue")) + 
...
ip<- ggplotly(p, tooltip = c( "y", "text")) %>%
  layout(
         hoverinfo = "text",
         separators = ","
  )
